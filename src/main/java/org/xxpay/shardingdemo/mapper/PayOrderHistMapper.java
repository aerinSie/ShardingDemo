...
public interface PayOrderHistMapper {
    ...
    @Insert("INSERT INTO t_pay_order_history(payOrderId, merchantId, amount,status, createTime," +
            "MerchantOrderNo,NotifyUrl) " +
            "VALUES(#{order.payOrderId}, #{order.merchantId}, #{order.amount}, #{order.status}," +
            " #{order.createTime},#{order.payOrderId} ,'test')")
    ...
    @Select("SELECT * FROM t_pay_order_history WHERE merchantId = #{merchantId}")
